        ...
        try {
            ...
            switch (strtolower($request->method())) {
                ...
                case 'put' :
                    ...
                    $code = 200; $message = __('companies.packages.create.success');
                    break;
                case 'patch' :
                    $valid = $this->validation->update($request);
                    $params = $this->repository->update($valid);
                    $code = 200; $message = __('companies.packages.update.success');
                    break;
                case 'delete' :
                    $valid = $this->validation->delete($request);
                    $params = $this->repository->delete($valid);
                    $code = 200; $message = __('companies.packages.delete.success');
                    break;
            }
